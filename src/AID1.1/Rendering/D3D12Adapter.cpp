...
bool D3D12Adapter::CreateGeometry() {
    ...
    // Создание UI буфера для прямоугольников (6 вершин = 2 треугольника)
    ...
    memcpy(m_cbvDataBegin, &m_objectConstants, sizeof(ObjectConstants));
...
void D3D12Adapter::SetObjectTransform(float x, float y, float scale, float rotation) {
    ...
    XMStoreFloat4x4(&m_objectConstants.worldMatrix, XMMatrixTranspose(worldMatrix));
...
void D3D12Adapter::DrawUIRect(float x, float y, float width, float height, float r, float g, float b, float a) {
    ...
    // Устанавливаем identity трансформацию для UI
    XMMATRIX identityMatrix = XMMatrixIdentity();
    XMStoreFloat4x4(&m_objectConstants.worldMatrix, XMMatrixTranspose(identityMatrix));
    memcpy(m_cbvDataBegin, &m_objectConstants, sizeof(ObjectConstants));
