...
void Logger::Log(LogLevel level, const std::string& message) {
    ...
    m_entries.push_back({ level, logMessage });
    if (m_entries.size() > 1000) {
        m_entries.erase(m_entries.begin(), m_entries.begin() + static_cast<std::ptrdiff_t>(m_entries.size() - 1000));
    }
...
std::vector<LogEntry> Logger::GetRecentEntries() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_entries;
}
