...
void CameraSystem::HandleInput(World* world, Entity cameraEntity, float deltaTime) {
    ...
    if (m_mouseCaptured) {
        ...
        int deltaX = static_cast<int>(mousePos.x - m_lastMouseX);
        int deltaY = static_cast<int>(mousePos.y - m_lastMouseY);
        ...
        ClientToScreen(m_hwnd, &center);
        SetCursorPos(center.x, center.y);
        m_lastMouseX = static_cast<float>((rect.right - rect.left) / 2);
        m_lastMouseY = static_cast<float>((rect.bottom - rect.top) / 2);
    }
