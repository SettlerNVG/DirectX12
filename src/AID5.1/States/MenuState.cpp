...
void MenuState::OnEnter() {
    ...
    SetWindowTextW(m_app->GetWindowHandle(), L"AID4.1 Physics Demo | Press SPACE to start");
}
...
void MenuState::Update(float deltaTime) {
    ...
    if (spacePressed && !spaceWasPressed) {
        LOG_INFO("SPACE pressed - starting physics demo!");
        m_app->ChangeState("PhysicsGameplayState");
    }
    
    if (mousePressed && !mouseWasPressed) {
        LOG_INFO("Mouse clicked - starting physics demo!");
        m_app->ChangeState("PhysicsGameplayState");
    }
...
void MenuState::Render() {
    ...
    if (hdc) {
        ...
        TextOutW(hdc, 150, 200, L"AID4.1 Physics", 14);
