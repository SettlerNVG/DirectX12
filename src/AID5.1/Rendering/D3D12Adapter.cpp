...
bool D3D12Adapter::CreateImGuiDescriptorHeap() {
    ...
    if (FAILED(m_device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&m_imguiSrvHeap)))) {
        LOG_ERROR("Failed to create ImGui SRV descriptor heap");
        return false;
    }
...
void D3D12Adapter::SetViewportRect(float x, float y, float width, float height) {
    const float minSize = 1.0f;
    const float maxLeft = (std::max)(0.0f, static_cast<float>(m_width) - minSize);
    const float maxTop = (std::max)(0.0f, static_cast<float>(m_height) - minSize);
    const float left = (std::min)(maxLeft, (std::max)(0.0f, x));
    const float top = (std::min)(maxTop, (std::max)(0.0f, y));
    const float right = (std::min)(static_cast<float>(m_width), left + (std::max)(minSize, width));
    const float bottom = (std::min)(static_cast<float>(m_height), top + (std::max)(minSize, height));

    m_viewport.TopLeftX = left;
    m_viewport.TopLeftY = top;
    m_viewport.Width = (std::max)(minSize, right - left);
    m_viewport.Height = (std::max)(minSize, bottom - top);
    m_viewport.MinDepth = 0.0f;
    m_viewport.MaxDepth = 1.0f;
...
void D3D12Adapter::ResetViewportRect() {
    m_viewport.TopLeftX = 0.0f;
    m_viewport.TopLeftY = 0.0f;
